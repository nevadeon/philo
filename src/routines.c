...
static void	_acquire_forks(t_data *data, t_thread_arg *arg)
{
	...
	print_philo_status2(data, arg->id, "\033[1;32mhas taken a fork\033[0m");
	print_philo_status2(data, arg->id, "\033[1;32mhas taken a fork\033[0m");
}

static void	_eat_meal(t_data *data, t_thread_arg *arg, int *meals_eaten)
{
	print_philo_status2(data, arg->id, "\033[1;31m       is eating\033[0m");
...
static void	_done_eating_process(t_data *data, t_thread_arg *arg)
{
	set_is_done_eating(arg, true);
...
void	*philo_routine(void *_arg)
{
	...
	meals_eaten = 0;
	if (data->nb_philos == 1)
		return (print_philo_status2(data, arg->id, "has taken a fork"), NULL);
	if (arg->id % 2 == 0)
		usleep(500);
	while (!stop_simulation(data)
		&& (meals_eaten < data->nb_meals_to_eat || data->nb_meals_to_eat == -1))
	{
		_acquire_forks(data, arg);
		_eat_meal(data, arg, &meals_eaten);
		print_philo_status2(data, arg->id, "\033[1;35m     is sleeping\033[0m");
		msleep(data->time_to_sleep_ms);
		print_philo_status2(data, arg->id, "\033[1;36m     is thinking\033[0m");
	}
	if (meals_eaten >= data->nb_meals_to_eat && data->nb_meals_to_eat != -1)
		_done_eating_process(data, arg);
	return (NULL);
}
...
void	*reaper_routine(void *_arg)
{
	...
	arg = _arg;
	data = arg->data;
	while (!stop_simulation(data) && !is_done_eating(arg))
	{
		...
		{
			print_philo_status2(data, arg->id, "died");
			set_stop_simulation(data, true);
			break ;
		}
		usleep(1000);
